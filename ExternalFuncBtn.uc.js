var gExternalFuncButtonM = {
	...
	loadSubMenu : function () {
		...
		for (var j = 0; j < this.toolbar.configs.length; j++) {
			...
			if (this.toolbar.configs[j].subdir && gExternalFuncButtonM.subdirPopupHash[this.toolbar.configs[j].subdir])
				gExternalFuncButtonM.subdirPopupHash[this.toolbar.configs[j].subdir].appendChild(configItems);
			else
	...
	updateToolbar : function () {
		...
		toolbars.forEach(function (toolbar) {
			...
			if (currentset.split(",").indexOf("assistantTools") < 0)
				return;
