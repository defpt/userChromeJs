...
var FullZoom = {
  ...
  getContentPrefs: function getContentPrefs(aWindow) {
    ...
    _handleMouseScrolled:function FullZoom__handleMouseScrolled(event) {
        ...
        if (event.shiftKey)
            pref += ".withshiftkey"; else if (event.ctrlKey)
            pref += ".withcontrolkey"; else if (event.altKey)
            pref += ".withaltkey"; else if (event.metaKey)
            pref += ".withmetakey"; else
            pref += ".withnokey";
    ...
    onContentPrefSet:function FullZoom_onContentPrefSet(aGroup, aName, aValue) {
        ...
        if (aGroup == this._cps.grouper.group(url))
            this._applyPrefToSetting(aName, aValue); else if (aGroup == null) {
    ...
    onContentPrefRemoved:function FullZoom_onContentPrefRemoved(aGroup, aName) {
        var url = this.convURI(gBrowser.currentURI);
        if (aGroup == this._cps.grouper.group(url))
            this._applyPrefToSetting(aName); else if (aGroup == null) {
    ...
    getApplied:function FullZoom_getApplied(aBrowser) {
        ...
        ZoomManager.useFullZoom = useFullZoom;
        if (value)
            ZoomManager.setZoomForBrowser(browser, this._ensureValid(value)); else
    ...
    reset:function FullZoom_reset() {
        if (typeof this.globalValue != "undefined")
            ZoomManager.zoom = this.globalValue; else
            ZoomManager.reset();
    ...
    _applyPrefToSetting:function FullZoom__applyPrefToSetting(aName, aValue, aBrowser) {
        ...
        if (aName == this.name) {
            try {
                if (resetZoom)
                    ZoomManager.setZoomForBrowser(browser, this.globalValue); else if (typeof aValue != "undefined")
                    ZoomManager.setZoomForBrowser(browser, this._ensureValid(aValue)); else
                    ZoomManager.setZoomForBrowser(browser, this.globalValue);
            } catch (ex) {
            }
            ...
            try {
                if (resetZoom)
                    ZoomManager.useFullZoom = this.globalMode; else if (typeof aValue != "undefined")
                    ZoomManager.useFullZoom = !!aValue; else
            ...
            try {
                if (resetZoom)
                    ZoomManager.useFullAuto = false; else if (typeof aValue != "undefined")
                    ZoomManager.useFullAuto = !!aValue; else
                    ZoomManager.useFullAuto = false;
            } catch (ex) {
            }
        }
    ...
    click:function (evt, fullZoom) {
        ...
        if (evt.button == 0 && evt.shiftKey) {
            ...
            if (document.getElementById("fullzoombtn") == btn || document.getElementById("fullzoombtn2") == btn)
                this.full = true; else if (document.getElementById("textzoombtn") == btn || document.getElementById("textzoombtn2") == btn)
                this.full = false;
            var popup = document.getElementById("fullZoomBtn_popup");
            // workaround Bug 622507
            popup.removeAttribute("height");
