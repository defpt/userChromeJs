...
(function MyMenuBtn() {
    ...
    var mMenus = [
		...
		{
            label: "MyFirefox文件夹",//这个是绝对路径，下面可设置
            command: "MyFirefoxfolder();",
		...
		{
            label: "Firefox备份文件夹",//这个是绝对路径，下面可设置
            command: "Backupfolder();",
...
function Backupfolder() {
    ...
    file.initWithPath("D:\\SkyDrive\\Firefox\\Firefox备份");file.launch();
}
//打开MyFirefox文件夹,这个是绝对路径，请自行设置
function MyFirefoxfolder() {
    var file = Components.classes["@mozilla.org/file/local;1"].createInstance(Components.interfaces.nsILocalFile);
    file.initWithPath("D:\\Program Files\\MyFirefox");file.launch();
}
//编辑userChrome.css 
