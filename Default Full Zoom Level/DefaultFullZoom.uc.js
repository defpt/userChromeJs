...
function() {
    this.defaultLv = 105;//默认的缩放级别
    this.Textmode = true;//只缩放文字
    this.localFolderSpecific = true;//是否应用于本地文件?
    this.fitToWidth = true;//自动适应窗口
    this.forceFitToWidth = false;
    this.fitToWidthPreserveTextSize = false;
    this.reserveSidebarWidth = true;//侧边栏宽度发生变化后,是否重新计算缩放
    this.maximum = 200;//自动适应窗口下的,最大值
    this.minimum = 50;//自动适应窗口下的,最小值
    this.ignoreImageDocument = true;//忽略图片类型的文件
    this.zoomValues = "0.8,0.9,1,1.1,1.2,1.3,1.4,1.5,2";//快捷菜单,最大值和最小值不能超过zoom.minPercent和zoom.maxPercent
...
var fullZoomBtn = {
    ...
    showZoomLevelInStatusbar: function() {
        ...
        if (ZoomManager.useFullZoom) label = "F" + label + "%";
        else label = "T" + label + "%";
